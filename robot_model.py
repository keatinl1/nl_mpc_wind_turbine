...
def export_lambda_robot_model() -> AcadosModel:
    # deprecated...
    ...
    # Define state variables
    Omega = SX.sym('Omega')  # Rotor speed
    Lambda = SX.sym('Lambda')  # Tip speed ratio
    theta = SX.sym('theta')  # Blade pitch angle
    Qg = SX.sym('Qg')  # Generator torque
    x = vertcat(Omega, Lambda, theta, Qg)
    ...
    model.t_label = "$t$ [s]"
    model.x_labels = ["$\\Omega$", "$\\lambda$", "$\\theta$", "$Q_g$"]
    model.u_labels = ["$u_1 (\\dot{\\theta})$", "$u_2 (\\dot{Q_g})$"]
