...
def plot_robot(shooting_nodes, u_max, U, X_traj, x_labels, u_labels, 
               time_label="$t$", latexify=True, plt_show=True):
    """
    Params:
    ...
    for i in range(nu):
        ...
        if u_max[i] is not None:
            plt.hlines(u_max[i], t[0], t[-1], linestyles="dashed", alpha=0.7, colors='orange')
            plt.hlines(-u_max[i], t[0], t[-1], linestyles="dashed", alpha=0.7, colors='orange')
